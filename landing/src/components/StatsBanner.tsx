...
export default function StatsBanner() {
  return (
    <section className="flex flex-col items-center gap-4 px-[120px] py-10 transition-colors"
      ...
      <div className="flex items-center justify-center gap-8 w-full">
        {stats.map((s, i) => (
          <div key={i} className="flex items-center gap-8">
            {i > 0 && <div className="w-px h-10" style={{ backgroundColor: "var(--color-border)" }} />}
            <div className="flex flex-col items-center gap-1">
              <span className="font-mono text-[28px] font-bold" style={{ color: "var(--color-text)" }}>
                <AnimatedCounter target={s.value} />
              </span>
              <span className="font-mono text-[11px]" style={{ color: "var(--color-text-secondary)" }}>{s.label}</span>
            </div>
          </div>
        ))}
      </div>
