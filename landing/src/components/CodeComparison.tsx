...
export default function CodeComparison() {
  return (
    <section className="flex flex-col items-center gap-12 px-[120px] py-20">
      ...
      <h2 className="text-[40px] font-bold text-center">From mainframe to microservice.</h2>
      ...
      <div className="flex w-full gap-0">
        ...
        <div className="flex-1 border border-[var(--color-border)] bg-[var(--color-bg)] p-4 flex flex-col gap-2">
          ...
          <pre className="font-mono text-xs text-[var(--color-text-secondary)] leading-[1.7] whitespace-pre">{cobol}</pre>
        </div>

        {/* Java */}
        <div className="flex-1 border border-l-0 border-[var(--color-border)] bg-[var(--color-bg)] p-4 flex flex-col gap-2">
          <div className="flex items-center gap-2">
          ...
          <div className="h-px bg-[var(--color-border)]" />
          <pre className="font-mono text-xs text-[var(--color-green)] leading-[1.7] whitespace-pre">{java}</pre>
        </div>
      </div>
      ...
      <div className="flex gap-4 justify-center w-full">
        {mappings.map((m, i) => (
          <div
            key={i}
            className="flex items-center gap-2 border border-[var(--color-border)] bg-[var(--color-bg)] px-4 py-2"
          >
            <span className="font-mono text-[11px] text-[var(--color-text-secondary)]">{m.from}</span>
            <span className="font-mono text-[11px] text-[var(--color-accent)]">&rarr;</span>
            <span className="font-mono text-[11px] font-semibold text-[var(--color-green)]">{m.to}</span>
          </div>
        ))}
      </div>
    </section>
