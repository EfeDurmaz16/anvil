...
export default function OpenSourceCTA() {
  return (
    <section
      className="flex flex-col items-center gap-6 md:gap-8 px-4 md:px-[120px] py-16 md:py-[100px] transition-colors"
      style={{ background: "radial-gradient(ellipse at center, var(--color-cta-gradient-from) 0%, var(--color-cta-gradient-to) 100%)" }}
    >
      <h2 className="text-2xl md:text-[40px] font-bold text-center leading-[1.2] max-w-[600px]" style={{ color: "var(--color-text)" }}>
      ...
      <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
        <a
          ...
          className="flex items-center justify-center gap-2.5 bg-[var(--color-accent)] text-black font-mono text-sm font-semibold px-7 py-3.5 hover:brightness-110 transition"
        >
          <Star size={18} />
          Star on GitHub
        </a>
